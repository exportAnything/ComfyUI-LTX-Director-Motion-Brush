...
let styleEl = document.getElementById("ltx-director-motion-brush-v2-styles");
if (!styleEl) {
  styleEl = document.createElement("style");
  styleEl.id = "ltx-director-motion-brush-v2-styles";
...
class TimelineEditor {
  constructor(node, container, domWidget) {
    ...
    console.log("[LTXDirectorMotionBrushV2 debug] Constructor: timelineDataWidget value:", this.timelineDataWidget?.value);
    ...
    console.log("[LTXDirectorMotionBrushV2 debug] Constructor: parsed timeline:", JSON.stringify(this.timeline));
  ...
  syncWidgetsAndUI() {
    console.log("[LTXDirectorMotionBrushV2 debug] syncWidgetsAndUI() called.");
  ...
  commitChanges(skipRender = false) {
    ...
    console.log("[LTXDirectorMotionBrushV2 debug] commitChanges: saving timelineDataWidget value:", jsonStr);
...
app.registerExtension({
  name: "LTXDirectorMotionBrushV2",
  async beforeRegisterNodeDef(nodeType, nodeData, app) {
    if (nodeData.name === "LTXDirectorMotionBrushV2") {
      ...
      nodeType.prototype.onNodeCreated = function () {
        ...
        const widget = this.addDOMWidget("timeline_motion_brush_v2_ui", "timeline_motion_brush_v2_ui", container, {
          getValue: () => "",
          setValue: () => { },
        });
        ...
        setTimeout(() => {
          try {
            ...
            console.error("[LTXDirectorMotionBrushV2] timeline editor init failed:", err);
          }
        }, 0);
      };
      ...
      nodeType.prototype.onConfigure = function (info) {
        ...
        console.log("[LTXDirectorMotionBrushV2 debug] onConfigure called. info.widgets_values:", info.widgets_values ? JSON.stringify(info.widgets_values) : "undefined");
        ...
        if (info.properties && info.properties.has_serialized_properties) {
          console.log("[LTXDirectorMotionBrushV2 debug] Restoring widgets from properties");
          if (this.widgets) {
            for (const w of this.widgets) {
          ...
          console.log("[LTXDirectorMotionBrushV2 debug] Restoring widgets via fallback name-based schema mapping");
          const SCHEMA_19 = [
            "start_frame", "end_frame", "duration_frames",
            "timeline_data", "motion_tracks_data", "use_custom_audio", "use_custom_motion", "inpaint_audio", "local_prompts", "segment_lengths",
            "epsilon", "frame_rate", "display_mode", "guide_strength", "custom_width", "custom_height",
            "resize_method", "divisible_by", "img_compression", "timeline_motion_brush_v2_ui"
          ];
          const SCHEMA_21_NO_INPAINT = [
            "start_second", "end_second", "duration_seconds", "start_frame", "end_frame", "duration_frames",
            "timeline_data", "motion_tracks_data", "local_prompts", "segment_lengths", "epsilon", "guide_strength",
          ...
          const SCHEMA_22_NO_INPAINT = [
            ...
            "timeline_data", "motion_tracks_data", "local_prompts", "segment_lengths", "epsilon", "guide_strength",
            "use_custom_audio", "use_custom_motion", "frame_rate", "display_mode", "custom_width", "custom_height",
            "resize_method", "divisible_by", "img_compression", "override_audio", "timeline_motion_brush_v2_ui"
          ];
        ...
        setTimeout(() => {
          if (this._timelineEditor) {
            console.log("[LTXDirectorMotionBrushV2 debug] setTimeout sync block called.");
            console.log("[LTXDirectorMotionBrushV2 debug] setTimeout: timelineDataWidget value:", this._timelineEditor.timelineDataWidget?.value);
            const tl = parseInitial(this._timelineEditor.timelineDataWidget?.value);
            this._timelineEditor.timeline = tl;
            this._timelineEditor.hydrateMotionTracksFromWidget();
            console.log("[LTXDirectorMotionBrushV2 debug] setTimeout: parsed timeline:", JSON.stringify(tl));
